class claseMemoriaLRU {
      ...
      if (this.MMU[i].id === puntero) {
        for (let j = 0; j < this.MMU[i].paginas.length; j++) {
          if (this.MMU[i].paginas[j].espacioEnMemoria === -1) {
            ...
            this.colaDePaginas.splice(
              this.MMU[i].paginas[j].espacioEnMemoria,
              1
            );
            this.colaDePaginas.push(
              this.RAM[this.MMU[i].paginas[j].espacioEnMemoria]
            );
          }
        }
        break;
  ...
  eliminaProcesoDeMemoria(idProceso) {
    ...
    this.colaDePaginas = this.colaDePaginas.filter(
      (item) => paginasABorrar.indexOf(item) === -1
    );
  }
