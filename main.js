...
function draw() {
  ...
  if(verificarSiProcesoEnTabla(parseInt(procesoSolicitado))===false){
            tablaDeProcesos.push({idProceso:parseInt(procesoSolicitado), accesoCompletado: false, tiempoDeVida:10, R:generateRandomIntegerRGB() , G: generateRandomIntegerRGB() , B: generateRandomIntegerRGB() })
          }
  background(255);
  dibujarCronometro();
